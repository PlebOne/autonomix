...
impl AppRow {
    pub fn new(app: &TrackedApp) -> Self {
        ...
        let uninstall_button = Button::builder()
            ...
            .visible(app.is_installed())
            .build();

        button_box.append(&uninstall_button);

        // Delete button (remove from tracking)
    ...
    /// Connect to uninstall button clicked
    pub fn connect_uninstall<F: Fn(i64) + Clone + 'static>(&self, f: F) {
        let app_id = self.app_id;
        self.uninstall_button.connect_clicked(move |_| {
            f(app_id);
        });
    }
